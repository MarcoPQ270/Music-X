...
public function Header() {
    ...
    // Title
    $this->Cell(0, 15, 'Informe de Artistas Music X', 0, false, 'C', 0, '', 0, false, 'M', 'M');
}
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach ($data as $row ){
			$this->Cell($w[0], 6, $row['idcompositor'], 'LR', 0, 'C', $fill);
			$this->Cell($w[1], 6, $row['nomc'], 'LR', 0, 'L', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
// column titles
$header = array('Id Compositor', 'Nombre Compositor');
...
$pdf->Output('ReporteCompositor.pdf', 'I');
