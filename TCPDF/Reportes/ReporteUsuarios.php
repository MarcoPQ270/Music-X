...
public function Header() {
    ...
    // Title
    $this->Cell(0, 15, 'Informe de Usuarios Music X', 0, false, 'C', 0, '', 0, false, 'M', 'M');
}
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach ($data as $row ){
			$this->Cell($w[0], 6, $row['correo'], 'LR', 0, 'L', $fill);
			$this->Cell($w[1], 6, $row['nomusuario'], 'LR', 0, 'L', $fill);
			$this->Cell($w[2], 6, $row['tipousr'], 'LR', 0, 'L', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
// column titles
$header = array('Correo', 'Nombre', 'Tip us');
