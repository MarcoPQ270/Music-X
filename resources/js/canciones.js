...
$("#add").validate({
    ...
    table.on('click', '.edit', function () {
        ...
        console.log(data);
        $("#nomcancion").val(data[0]);
        $("#fechapub").val(data[1]);
        $("#genero").val(data[2]);
        $("#artista").val(data[3]);
        $("#imagen").val(data[4]);
        $("#pk").val(data.DT_RowId);
    ...
    $("#print-record").on("click",function(){
        document.location.href = "http://localhost/pruebas/TCPDF/Reportes/ReporteCanciones.php"
    });
...
function cargaCanciones(){
    var parametros = "";
    $.ajax({
        type:"post",
        url:"llenaArrayCanciones.php",
        dataType:'json',
        data:parametros,
...
function saveData(){
    ...
    var gen=$("#idtipgenero option:selected").text();
    var comp=$("#idtipcom option:selected").text();
    parametros.append("gen",gen);
    parametros.append("comp",comp);
    $.ajax({
        type:"post",
        url:sURL,
        contentType: false,
